...
carries its identity (`--project 'c4s-plugin-scaffold' --workspace 'default'`), so it works from any cwd. Do NOT `cd`
...
```sh
c4s catalog --project 'c4s-plugin-scaffold' --workspace 'default'                                   # entity types + schemas
...
the implementer must change and why>" --ct brief --source analysis --project 'c4s-plugin-scaffold' --workspace 'default'
```
